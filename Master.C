...
//	pkt->Show();
	...
	if(pkt->GetError() == 0){
		...
		if(bt_flag > 0 && tmpdstmacaddr != "68:b5:99:76:4a:60"){
			//cout << "\n\n" << recom_flag << " " << tmpsrcmacaddr << " " << tmpdstmacaddr << " " << tmptimestamp<<"\n\n";
			MASTER_DEBUG(MSG("browsingtime"));
			browsingtime->Proc(tmpdstmacaddr);
			bt_flag = 0;
		}

		if(recom_flag > 0){
			MASTER_DEBUG(MSG("recomendation"))
			cf->Proc(tmpdstmacaddr);
