...
//				pkt->GetStream()->SetSaveFlag();
				...
				char timebuff1[80];
				strftime(timebuff1, 80, "%Y-%m-%d %H:%M:%S", tmp);
				int milli = tmp_time.tv_usec/1000;
				char timebuff2[84];
				sprintf(timebuff2, "%s.%d", timebuff1, milli);

				ostringstream oss;
				oss << timebuff2;
				//oss << tmp->tm_year+1900 <<"-"<< tmp->tm_mon+1 <<"-"<<tmp->tm_mday <<" "<<tmp->tm_hour<<":"<<tmp->tm_min<<":"<<tmp->tm_sec<< "." << (pkt->GetStream()->GetTimestamp()).tv_usec;
				//oss << tmp->tm_year+1900 <<"-"<< tmp->tm_mon+1 <<"-"<<tmp->tm_mday <<" "<<tmp->tm_hour<<":"<<tmp->tm_min<<":"<<tmp->tm_sec;
				string tstamp = oss.str();
				
				//struct tm tm;
				ostringstream tss;
				tss << tmp_time.tv_sec << "." << tmp_time.tv_usec;
				string epoch_time = tss.str();
...
//*/
					...
					//<< "(default'" << pkt->GetStream()->GetStreamId() << "','" << (*it)->GetRuleId() << "','" \
						...
						oss << "insert into save_result(stream_id, rule_id, pattern, pattern_len, place,timestamp, src_ip, dst_ip, src_port, dst_port, src_mac_addr, dst_mac_addr, epoch_time, result) values "\
...
//					cout << query << endl;
					...
					if((*it)->GetPRule()->GetPreFilterPattern() == "<title" || (*it)->GetPRule()->GetPreFilterPattern() == "<TITLE"){
						//BrowsingTime *browsingtime = new BrowsingTime;
						//CollaborativeFiltering *cf = new CollaborativeFiltering;
						//browsingtime->Proc(3, pkt->GetSrcMacAddr(), tstampn, "short");
						//cf->Proc("");

						master->bt_flag++;
						master->tmpsrcmacaddr = pkt->GetSrcMacAddr();
						master->tmpdstmacaddr = pkt->GetDstMacAddr();
						master->tmpsrcip = pkt->GetSrcIPStr();
						master->tmpdstip = pkt->GetDstIPStr();
						master->tmptimestamp = tstamp;
					}
#endif //RECOMMENDATION
